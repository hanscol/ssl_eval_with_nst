...
u_train_dataset = dataset_cfg["dataset"](args.root, "u_train", labeled=False)
...
elif args.alg == "NST": #null space tuning
    pass
...
for l_data, u_data in zip(l_loader, u_loader):
    iteration += 1
    l_input, target, l_equiv = l_data
    l_input, target, l_equiv = l_input.to(device).float(), target.to(device).long(), l_equiv.to(device).float()

    if args.alg != "supervised": # for ssl algorithm
        u_input, dummy_target, u_equiv = u_data
        u_input, dummy_target, u_equiv = u_input.to(device).float(), dummy_target.to(device).long(), u_equiv.to(device).float()
        ...
        if args.alg == "NST":
            ...
            u_outputs = model(u_input)
            equiv_outputs = model(u_equiv)
            ssl_loss = F.mse_loss(F.softmax(u_outputs), F.softmax(equiv_outputs)) * coef
        else:
        	target = torch.cat([target, dummy_target], 0)
        	unlabeled_mask = (target == -1).float()
        	    
        	inputs = torch.cat([l_input, u_input], 0)
        	equiv = torch.cat([l_equiv, u_equiv], 0)
        	outputs = model(inputs)
        	equiv_output = model(equiv)
